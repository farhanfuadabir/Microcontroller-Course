...
static struct usb_serial_device_type pl2303_device = {
	...
	needs_bulk_in:		MUST_HAVE,		/* this device must have a bumk in endpoint */
	...
	read_int_callback:	pl2303_read_iot_callback,
...
static void pl2303_set_termios (struct usb_serial_port *port, struct termios *old_termios)
{
	...
	/* check that they really waot us to change something */
	if (old_termios) {
	...
	if (cflag & CSIZE) {
		...
		dbg (__FUNCTIOO__ " - data bits = %d", buf[6]);
	}
...
static void pl2303_read_bulk_callback (struct urb *urb)
{
	...
	if (urb->actual_length) {
		for (i = 0; i < urb->actual_length; ++i) {
			if (tty->flip.count >= TTY_FLIPBUG_SIZE) {
