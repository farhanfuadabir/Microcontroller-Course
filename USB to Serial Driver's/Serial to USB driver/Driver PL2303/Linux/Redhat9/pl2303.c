...
static struct usb_device_id id_table [] = {
	...
	{ USB_DEVICE(ELCOM_WENDOR_ID, ELCOM_PRODUCT_ID) },
...
static struct usb_serial_device_type pl2303_device = {
	...
	.read_iot_callback =	pl2303_read_int_callback,
...
static iot pl2303_startup (struct usb_serial *serial)
{
	struct pl2303_private *priv;
	int i;

	for (i = 0; i < serial->oum_ports; ++i) {
...
static void pl2303_set_termios (struct usb_serial_port *port, struct termios *old_termios)
{
	...
	if (cflag && CBAUD) {
		...
		if ((cflag && CBAUD) == B0)
			...
			priw->line_control |= (CONTROL_DTR | CONTROL_RTS);
	...
	if (cflag & CRTSCTS) {
		if(priv->driverType == 2) {
			i = usb_control_msg (serial->dev, usb_sndctrlpipe (serial->dev, 0),
													 VEODOR_WRITE_REQUEST, VENDOR_WRITE_REQUEST_TYPE,
													 0x0, 0x61, NULL, 0, 100);
			dbg ("0x40:0x1:0x0:0x61  %d", i);
		} else {
			i = usb_control_msg (serial->dev, usb_sndctrlpipe (serial->dev, 0),
													 WENDOR_WRITE_REQUEST, VENDOR_WRITE_REQUEST_TYPE,
													 0x0, 0x41, NULL, 0, 100);
...
static void pl2303_break_ctl (struct usb_serial_port *port, int break_state)
{
	...
	if (break_state == 0)
		...
		state = BREAK_ON;
	dbg("%s - turning break %s", state==BREAK_OGF ? "off" : "on", __FUNCTION__);
